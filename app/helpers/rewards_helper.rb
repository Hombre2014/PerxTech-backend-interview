module RewardsHelper
  def check_5_percent_reward
    ...
    @level_2_users.each do |user|
      @big_purchases = Purchase.where(user_id: user.id, amount: 100..400).order(date: :asc).all
  ...
  def free_movie_tickets_reward(users_level_two)
    users_level_two.each do |user|
      ...
      first_purchase = Purchase.where(user_id: user.id).order(date: :asc).first || nil
      if !first_purchase.nil?
        ...
        all_purchases_within_2_months.each do |purchase|
          next if purchase.checked_for_free_movie_tickets
